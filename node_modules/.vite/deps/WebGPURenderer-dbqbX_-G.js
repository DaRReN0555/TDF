import { BindGroup, Buffer, BufferUsage, CanvasSource, DOMAdapter, ExtensionType, Matrix, RendererType, STENCIL_MODES, Shader, State, Texture, TextureSource, UniformGroup, colorBit, compileHighShaderGpuProgram, createIdFromString, extensions, fastCopy, generateTextureBatchBit, localUniformBit, localUniformBitGroup2, roundPixelsBit, warn } from "./colorToUniform-CUTLKdLN.js";
import { AbstractRenderer, BufferResource, CLEAR, GpuStencilModesToPixi, RenderTargetSystem, SharedRenderPipes, SharedSystems, UboSystem, createUboSyncFunction, ensureAttributes, textureBit, uboSyncFunctionsWGSL } from "./SharedSystems-hjSX02JQ.js";
...
var UboBatch = class {
	constructor({ minUniformOffsetAlignment: minUniformOffsetAlignment$1 }) {
		this._minUniformOffsetAlignment = 256;
		this.byteIndex = 0;
		this._minUniformOffsetAlignment = minUniformOffsetAlignment$1;
...
var GpuRenderTargetAdaptor = class {
	...
	startRenderPass(renderTarget, clear = true, clearColor, viewport) {
		...
		gpuRenderTarget.descriptor = this.getDescriptor(renderTarget, clear, clearColor);
	...
	*/
	...
	getDescriptor(renderTarget, clear, clearValue) {
		...
		const colorAttachments = renderTarget.colorTextures.map((texture, i) => {
			...
			let view;
			let resolveTarget;
			if (context) view = context.getCurrentTexture().createView();
			else view = this._renderer.texture.getGpuSource(texture).createView({ mipLevelCount: 1 });
	...
	*/
	getColorTargets(state) {
		return [{
			format: "bgra8unorm",
			writeMask: 0,
			blend: GpuBlendModesToPixi[state.blendMode] || GpuBlendModesToPixi.normal
		}];
	}
	destroy() {
		this.gpu = null;
...
var GpuTextureSystem = class {
	...
	*/
	initSource(source) {
		if (this._gpuSources[source.uid]) return this._gpuSources[source.uid];
