import { BatchableSprite, BigPool, BindGroup, Bounds, Buffer, BufferUsage, CanvasSource, Color, Container, DOMAdapter, DefaultBatcher, ExtensionType, Filter, FilterEffect, GlProgram, GlobalResourceRegistry, GpuProgram, Matrix, Point, Rectangle, RenderGroup, RendererType, STENCIL_MODES, Sprite, State, Texture, TextureMatrix, TexturePool, TextureSource, TextureStyle, Ticker, UPDATE_BLEND, UPDATE_COLOR, UPDATE_VISIBLE, UniformGroup, color32BitToUniform, deprecation, eventemitter3_default, extensions, getAttributeInfoFromFormat, getGlobalBounds, getLocalBounds, multiplyColors, uid as uid$1, v8_0_0, warn } from "./colorToUniform-CUTLKdLN.js";
...
var _AbstractRenderer = class _AbstractRenderer$1 extends eventemitter3_default {
	/**
	...
	*/
	async init(options = {}) {
		...
		options = {
			..._AbstractRenderer$1.defaultOptions,
			...options
		};
	...
	render(args, deprecated) {
		...
		if (options.clearColor) options.clearColor = Array.isArray(options.clearColor) && options.clearColor.length === 4 ? options.clearColor : Color.shared.setValue(options.clearColor).toArray();
...
var _BatcherPipe = class _BatcherPipe$1 {
	...
	addToBatch(batchableObject, instructionSet) {
		if (this._activeBatch.name !== batchableObject.batcherName) {
			...
			if (!batch) {
				batch = this._activeBatches[batchableObject.batcherName] = _BatcherPipe$1.getBatcher(batchableObject.batcherName);
				batch.begin();
			}
...
var _RenderTarget = class _RenderTarget$1 {
	/**
	...
	*/
	constructor(descriptor = {}) {
		...
		*/
		...
		descriptor = {
			..._RenderTarget$1.defaultOptions,
			...descriptor
		};
	...
	onSourceResize(source$1) {
		this.resize(source$1.width, source$1.height, source$1._resolution, true);
	}
	/**
...
var RenderTargetSystem = class {
	...
	*/
	bind(renderSurface, clear = true, clearColor, frame) {
		...
		const source$1 = renderTarget.colorTexture;
		const viewport = this.viewport;
		const pixelWidth = source$1.pixelWidth;
		const pixelHeight = source$1.pixelHeight;
		if (!frame && renderSurface instanceof Texture) frame = renderSurface.frame;
		if (frame) {
			const resolution = source$1._resolution;
		...
		calculateProjection(this.projectionMatrix, 0, 0, viewport.width / source$1.resolution, viewport.height / source$1.resolution, !renderTarget.isRoot);
...
function updateColorBlendVisibility(container, parent, updateFlags) {
	...
	if (updateFlags & UPDATE_BLEND) container.groupBlendMode = container.localBlendMode === "inherit" ? parent.groupBlendMode : container.localBlendMode;
	if (updateFlags & UPDATE_VISIBLE) container.globalDisplayStatus = container.localDisplayStatus & parent.globalDisplayStatus;
...
var _BackgroundSystem = class _BackgroundSystem$1 {
	constructor() {
	...
	*/
	init(options) {
		options = {
			..._BackgroundSystem$1.defaultOptions,
			...options
		};
...
var _ExtractSystem = class _ExtractSystem$1 {
	...
	*/
	async base64(options) {
		options = this._normalizeOptions(options, _ExtractSystem$1.defaultImageOptions);
...
var _RenderableGCSystem = class _RenderableGCSystem$1 {
	/**
	...
	*/
	init(options) {
		options = {
			..._RenderableGCSystem$1.defaultOptions,
			...options
		};
...
var _TextureGCSystem = class _TextureGCSystem$1 {
	/** @param renderer - The renderer this System works for. */
	constructor(renderer) {
		this._renderer = renderer;
		this.count = 0;
	...
	init(options) {
		options = {
			..._TextureGCSystem$1.defaultOptions,
			...options
		};
...
var _ViewSystem = class _ViewSystem$1 {
	/**
