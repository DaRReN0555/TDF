import { BatchableSprite, BigPool, BindGroup, Bounds, Buffer, BufferImageSource, BufferUsage, CanvasSource, Color, Container, DOMAdapter, DefaultBatcher, ExtensionType, Filter, Geometry, GlProgram, GpuProgram, InstructionSet, Matrix, Point, Rectangle, RendererType, Shader, Sprite, State, Texture, TexturePool, TextureSource, TextureStyle, Ticker, UPDATE_PRIORITY, UniformGroup, ViewContainer, ViewableBuffer, __commonJS, __toESM, boundsPool, color32BitToUniform, colorBit, colorBitGl, compileHighShaderGlProgram, compileHighShaderGpuProgram, definedProps, deprecation, earcut, eventemitter3_default, extensions, generateTextureBatchBit, generateTextureBatchBitGl, getAdjustedBlendModeBlend, getAttributeInfoFromFormat, getBatchSamplersUniformGroup, getGlobalBounds, getLocalBounds, localUniformBit, localUniformBitGl, matrixPool, multiplyHexColors, nextPow2, roundPixelsBit, roundPixelsBitGl, uid, updateQuadBounds, v8_0_0, v8_3_4, warn } from "./colorToUniform-CUTLKdLN.js";
import { CanvasPool, getTextureBatchBindGroup } from "./CanvasPool-Dn3EAtFy.js";
...
function addMaskLocalBounds(mask, bounds, localRoot) {
	...
	mask.measurable = true;
	const tempMatrix$1 = matrixPool.get().identity();
	const relativeMask = getMatrixRelativeToParent(mask, localRoot, tempMatrix$1);
	getLocalBounds(mask, boundsToMask, relativeMask);
	mask.measurable = false;
	bounds.addBoundsMask(boundsToMask);
	matrixPool.return(tempMatrix$1);
...
var _VideoSource = class _VideoSource$1 extends TextureSource {
	constructor(options) {
		super(options);
	...
	*/
	async load() {
		...
		const source$1 = this.resource;
		const options = this.options;
		if ((source$1.readyState === source$1.HAVE_ENOUGH_DATA || source$1.readyState === source$1.HAVE_FUTURE_DATA) && source$1.width && source$1.height) source$1.complete = true;
		source$1.addEventListener("play", this._onPlayStart);
		source$1.addEventListener("pause", this._onPlayStop);
		source$1.addEventListener("seeked", this._onSeeked);
		if (!this._isSourceReady()) {
			if (!options.preload) source$1.addEventListener("canplay", this._onCanPlay);
			source$1.addEventListener("canplaythrough", this._onCanPlayThrough);
			source$1.addEventListener("error", this._onError, true);
	...
	*/
	_isSourcePlaying() {
		const source$1 = this.resource;
		return !source$1.paused && !source$1.ended;
	}
	/**
	...
	*/
	...
	_mediaReady() {
		...
		if (this.isValid) {
			this.isReady = true;
			this.resize(source$1.videoWidth, source$1.videoHeight);
		}
		this._msToNextUpdate = 0;
		this.updateFrame();
		this._msToNextUpdate = 0;
	...
	destroy() {
		...
		const source$1 = this.resource;
		if (source$1) {
			source$1.removeEventListener("play", this._onPlayStart);
			source$1.removeEventListener("pause", this._onPlayStop);
			source$1.removeEventListener("seeked", this._onSeeked);
			source$1.removeEventListener("canplay", this._onCanPlay);
			source$1.removeEventListener("canplaythrough", this._onCanPlayThrough);
			source$1.removeEventListener("error", this._onError, true);
			source$1.pause();
			source$1.src = "";
			source$1.load();
		}
	...
	resolveBundle(bundleIds) {
		...
		bundleIds.forEach((bundleId) => {
			...
			if (assetNames) {
				...
				out$1[bundleId] = assets;
			}
		});
		return singleAsset ? out$1[bundleIds[0]] : out$1;
	}
	/**
	...
	*/
	resolveUrl(key) {
		...
		if (typeof key !== "string") {
			const out$1 = {};
			for (const i in result) out$1[i] = result[i].src;
			return out$1;
		}
		return result.src;
	}
...
var _Spritesheet = class _Spritesheet$1 {
	...
	*/
	parse() {
		return new Promise((resolve) => {
			...
			this._batchIndex = 0;
			if (this._frameKeys.length <= _Spritesheet$1.BATCH_SIZE) {
	...
	*/
	_processFrames(initialFrameIndex) {
		...
		const maxFrames = _Spritesheet$1.BATCH_SIZE;
	...
	_nextBatch() {
		this._processFrames(this._batchIndex * _Spritesheet$1.BATCH_SIZE);
		this._batchIndex++;
		setTimeout(() => {
			if (this._batchIndex * _Spritesheet$1.BATCH_SIZE < this._frameKeys.length) this._nextBatch();
			else {
				this._processAnimations();
				this._parseComplete();
			}
...
function getCacheableAssets(keys, asset, ignoreMultiPack) {
	...
	Object.keys(asset.textures).forEach((key) => {
		out$1[`${asset.cachePrefix}${key}`] = asset.textures[key];
	});
	if (!ignoreMultiPack) {
		const basePath = path.dirname(keys[0]);
		asset.linkedSheets.forEach((item, i) => {
			const out2 = getCacheableAssets([`${basePath}/${asset.data.meta.related_multi_packs[i]}`], item, true);
			Object.assign(out$1, out2);
		});
	}
	return out$1;
}
...
var _CanvasTextMetrics = class _CanvasTextMetrics$1 {
	/**
	...
	*/
	static get experimentalLetterSpacingSupported() {
		let result = _CanvasTextMetrics$1._experimentalLetterSpacingSupported;
		if (result === void 0) {
			const proto = DOMAdapter.get().getCanvasRenderingContext2D().prototype;
			result = _CanvasTextMetrics$1._experimentalLetterSpacingSupported = "letterSpacing" in proto || "textLetterSpacing" in proto;
		}
		return result;
	}
	/**
	...
	*/
	static measureText(text = " ", style, canvas = _CanvasTextMetrics$1._canvas, wordWrap = style.wordWrap) {
		const textKey = `${text}-${style.styleKey}-wordWrap-${wordWrap}`;
		if (_CanvasTextMetrics$1._measurementCache.has(textKey)) return _CanvasTextMetrics$1._measurementCache.get(textKey);
		const font = fontStringFromTextStyle(style);
		const fontProperties = _CanvasTextMetrics$1.measureFont(font);
		if (fontProperties.fontSize === 0) {
			fontProperties.fontSize = style.fontSize;
			fontProperties.ascent = style.fontSize;
		}
		const context = _CanvasTextMetrics$1.__context;
		context.font = font;
		const lines = (wordWrap ? _CanvasTextMetrics$1._wordWrap(text, style, canvas) : text).split(/(?:\r\n|\r|\n)/);
		const lineWidths = new Array(lines.length);
		let maxLineWidth = 0;
		for (let i = 0; i < lines.length; i++) {
			const lineWidth = _CanvasTextMetrics$1._measureText(lines[i], style.letterSpacing, context);
		...
		const measurements = new _CanvasTextMetrics$1(text, style, width, height, lines, lineWidths, lineHeight + style.leading, maxLineWidth, fontProperties);
		_CanvasTextMetrics$1._measurementCache.set(textKey, measurements);
		return measurements;
	}
	static _measureText(text, letterSpacing, context) {
		let useExperimentalLetterSpacing = false;
		if (_CanvasTextMetrics$1.experimentalLetterSpacingSupported) if (_CanvasTextMetrics$1.experimentalLetterSpacing) {
		...
		let boundsWidth = metrics.actualBoundingBoxRight - actualBoundingBoxLeft;
		if (metricWidth > 0) if (useExperimentalLetterSpacing) {
			metricWidth -= letterSpacing;
			boundsWidth -= letterSpacing;
		} else {
			const val = (_CanvasTextMetrics$1.graphemeSegmenter(text).length - 1) * letterSpacing;
			metricWidth += val;
			boundsWidth += val;
		}
	...
	*/
	static _wordWrap(text, style, canvas = _CanvasTextMetrics$1._canvas) {
		...
		const collapseSpaces = _CanvasTextMetrics$1._collapseSpaces(whiteSpace);
		const collapseNewlines = _CanvasTextMetrics$1._collapseNewlines(whiteSpace);
		let canPrependSpaces = !collapseSpaces;
		const wordWrapWidth = style.wordWrapWidth + letterSpacing;
		const tokens = _CanvasTextMetrics$1._tokenize(text);
		for (let i = 0; i < tokens.length; i++) {
			let token = tokens[i];
			if (_CanvasTextMetrics$1._isNewline(token)) {
				if (!collapseNewlines) {
					lines += _CanvasTextMetrics$1._addLine(line);
			...
			if (collapseSpaces) {
				const currIsBreakingSpace = _CanvasTextMetrics$1.isBreakingSpace(token);
				const lastIsBreakingSpace = _CanvasTextMetrics$1.isBreakingSpace(line[line.length - 1]);
				if (currIsBreakingSpace && lastIsBreakingSpace) continue;
			}
			const tokenWidth = _CanvasTextMetrics$1._getFromCache(token, letterSpacing, cache, context);
			if (tokenWidth > wordWrapWidth) {
				...
				if (_CanvasTextMetrics$1.canBreakWords(token, style.breakWords)) {
					const characters = _CanvasTextMetrics$1.wordWrapSplit(token);
					for (let j = 0; j < characters.length; j++) {
						...
						while (characters[j + k]) {
							...
							if (!_CanvasTextMetrics$1.canBreakChars(lastChar, nextChar, token, j, style.breakWords)) char += nextChar;
							else break;
							lastChar = nextChar;
							k++;
						}
						j += k - 1;
						const characterWidth = _CanvasTextMetrics$1._getFromCache(char, letterSpacing, cache, context);
					...
					lines += _CanvasTextMetrics$1._addLine(token, !isLastToken);
					canPrependSpaces = false;
					line = "";
					width = 0;
				}
				...
				if (line.length > 0 || !_CanvasTextMetrics$1.isBreakingSpace(token) || canPrependSpaces) {
					line += token;
					width += tokenWidth;
				}
			}
		}
		lines += _CanvasTextMetrics$1._addLine(line, false);
		return lines;
	}
	/**
	...
	*/
	static _addLine(line, newLine = true) {
		line = _CanvasTextMetrics$1._trimRight(line);
		line = newLine ? `${line}
` : line;
		return line;
	}
	...
	*/
	static _getFromCache(key, letterSpacing, cache, context) {
		...
		if (typeof width !== "number") {
			width = _CanvasTextMetrics$1._measureText(key, letterSpacing, context) + letterSpacing;
			cache[key] = width;
		}
		return width;
	}
	...
	*/
	static _trimRight(text) {
		...
		for (let i = text.length - 1; i >= 0; i--) {
			const char = text[i];
			if (!_CanvasTextMetrics$1.isBreakingSpace(char)) break;
			text = text.slice(0, -1);
		}
		return text;
	}
	...
	*/
	static _isNewline(char) {
		...
		return _CanvasTextMetrics$1._newlines.includes(char.charCodeAt(0));
	}
	/**
	...
	*/
	static isBreakingSpace(char, _nextChar) {
		if (typeof char !== "string") return false;
		return _CanvasTextMetrics$1._breakingSpaces.includes(char.charCodeAt(0));
	}
	/**
	...
	*/
	static _tokenize(text) {
		...
		for (let i = 0; i < text.length; i++) {
			...
			if (_CanvasTextMetrics$1.isBreakingSpace(char, nextChar) || _CanvasTextMetrics$1._isNewline(char)) {
				if (token !== "") {
					tokens.push(token);
					token = "";
				}
	...
	*/
	static wordWrapSplit(token) {
		return _CanvasTextMetrics$1.graphemeSegmenter(token);
	}
	/**
	...
	*/
	static measureFont(font) {
		if (_CanvasTextMetrics$1._fonts[font]) return _CanvasTextMetrics$1._fonts[font];
		const context = _CanvasTextMetrics$1._context;
		context.font = font;
		const metrics = context.measureText(_CanvasTextMetrics$1.METRICS_STRING + _CanvasTextMetrics$1.BASELINE_SYMBOL);
		const properties = {
			ascent: metrics.actualBoundingBoxAscent,
			descent: metrics.actualBoundingBoxDescent,
			fontSize: metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent
		};
		_CanvasTextMetrics$1._fonts[font] = properties;
		return properties;
	}
	/**
	...
	*/
	static clearMetrics(font = "") {
		if (font) delete _CanvasTextMetrics$1._fonts[font];
		else _CanvasTextMetrics$1._fonts = {};
	}
	/**
	...
	*/
	static get _canvas() {
		if (!_CanvasTextMetrics$1.__canvas) {
			let canvas;
			try {
				const c = new OffscreenCanvas(0, 0);
				if (c.getContext("2d", contextSettings)?.measureText) {
					_CanvasTextMetrics$1.__canvas = c;
					return c;
				}
			...
			_CanvasTextMetrics$1.__canvas = canvas;
		}
		return _CanvasTextMetrics$1.__canvas;
	}
	/**
	* TODO: this should be private, but isn't because of backward compat, will fix later.
	* @ignore
	*/
	static get _context() {
		if (!_CanvasTextMetrics$1.__context) _CanvasTextMetrics$1.__context = _CanvasTextMetrics$1._canvas.getContext("2d", contextSettings);
		return _CanvasTextMetrics$1.__context;
	}
};
/**
...
_CanvasTextMetrics.graphemeSegmenter = (() => {
	if (typeof Intl?.Segmenter === "function") {
		...
		return (s) => {
			...
			const result = [];
			let i = 0;
			for (const segment$1 of segments) result[i++] = segment$1.segment;
			return result;
		};
	}
	return (s) => [...s];
...
_CanvasTextMetrics._newlines = [10, 13];
...
var _FillGradient = class _FillGradient$1 {
	constructor(...args) {
		/**
		...
		*/
		...
		options = {
			...options.type === "radial" ? _FillGradient$1.defaultRadialOptions : _FillGradient$1.defaultLinearOptions,
...
var require_parse_svg_path = /* @__PURE__ */ __commonJS({ "node_modules/parse-svg-path/index.js": ((exports, module) => {
	...
	* @return {Array}
	*/
	function parse$1(path$1) {
		var data = [];
		path$1.replace(segment, function(_, command, args) {
...
function parseSVGPath(svgPath, path$1) {
	...
	for (let i = 0; i < commands.length; i++) {
		...
		switch (type) {
			...
			case "C":
				lastX = data[5];
				lastY = data[6];
				path$1.bezierCurveTo(data[1], data[2], data[3], data[4], lastX, lastY);
				break;
			case "c":
				path$1.bezierCurveTo(lastX + data[1], lastY + data[2], lastX + data[3], lastY + data[4], lastX + data[5], lastY + data[6]);
				lastX += data[5];
				lastY += data[6];
				break;
			case "S":
				lastX = data[3];
				lastY = data[4];
				path$1.bezierCurveToShort(data[1], data[2], lastX, lastY);
				break;
			case "s":
				path$1.bezierCurveToShort(lastX + data[1], lastY + data[2], lastX + data[3], lastY + data[4]);
				lastX += data[3];
				lastY += data[4];
				break;
			case "Q":
				lastX = data[3];
				lastY = data[4];
				path$1.quadraticCurveTo(data[1], data[2], lastX, lastY);
				break;
			case "q":
				path$1.quadraticCurveTo(lastX + data[1], lastY + data[2], lastX + data[3], lastY + data[4]);
				lastX += data[3];
				lastY += data[4];
				break;
			case "T":
				lastX = data[1];
				lastY = data[2];
				path$1.quadraticCurveToShort(lastX, lastY);
				break;
			case "t":
				lastX += data[1];
				lastY += data[2];
			...
			case "A":
				lastX = data[6];
				lastY = data[7];
				path$1.arcToSvg(data[1], data[2], data[3], data[4], data[5], lastX, lastY);
				break;
			case "a":
				lastX += data[6];
				lastY += data[7];
...
var Circle = class Circle {
	...
	*/
	getBounds(out$1) {
		out$1 || (out$1 = new Rectangle());
		out$1.x = this.x - this.radius;
		out$1.y = this.y - this.radius;
		out$1.width = this.radius * 2;
		out$1.height = this.radius * 2;
		return out$1;
	}
	/**
...
var Ellipse = class Ellipse {
	...
	*/
	getBounds(out$1) {
		out$1 || (out$1 = new Rectangle());
		out$1.x = this.x - this.halfWidth;
		out$1.y = this.y - this.halfHeight;
		out$1.width = this.halfWidth * 2;
		out$1.height = this.halfHeight * 2;
		return out$1;
	}
	/**
...
var Polygon = class Polygon {
	...
	*/
	isClockwise() {
		...
		const length$1 = points.length;
		for (let i = 0; i < length$1; i += 2) {
			const x1 = points[i];
			const y1 = points[i + 1];
			const x2 = points[(i + 2) % length$1];
			const y2 = points[(i + 3) % length$1];
			area += (x2 - x1) * (y2 + y1);
		}
		return area < 0;
	}
	...
	*/
	contains(x, y) {
		let inside = false;
		const length$1 = this.points.length / 2;
		for (let i = 0, j = length$1 - 1; i < length$1; j = i++) {
	...
	*/
	getBounds(out$1) {
		...
		out$1.x = minX;
		out$1.width = maxX - minX;
		out$1.y = minY;
		out$1.height = maxY - minY;
		return out$1;
	}
	/**
...
var RoundedRectangle = class RoundedRectangle {
	...
	*/
	getBounds(out$1) {
		...
		out$1.x = this.x;
		out$1.y = this.y;
		out$1.width = this.width;
		out$1.height = this.height;
		return out$1;
	}
	/**
...
function buildLine(points, lineStyle, flipAlignment, closed, vertices, indices) {
	...
	const length$1 = points.length / 2;
	...
	for (let i = 1; i < length$1 - 1; ++i) {
		...
		if (pDist <= smallerInsideDiagonalSq) if (style.join === "bevel" || pDist / widthSquared > miterLimitSquared) {
			if (clockwise) {
				verts.push(imx, imy);
				verts.push(x1 + perpX * outerWeight, y1 + perpY * outerWeight);
				verts.push(imx, imy);
	...
	x0 = points[(length$1 - 2) * 2];
	y0 = points[(length$1 - 2) * 2 + 1];
	x1 = points[(length$1 - 1) * 2];
	y1 = points[(length$1 - 1) * 2 + 1];
	perpX = -(y0 - y1);
	perpY = x0 - x1;
	dist = Math.sqrt(perpX * perpX + perpY * perpY);
	perpX /= dist;
...
function buildPixelLine(points, closed, vertices, indices) {
	...
	for (let i = 0; i < length$1 - 1; i++) indices.push(indexStart + i, indexStart + i + 1);
	if (closePath) indices.push(indexStart + length$1 - 1, indexStart);
}
...
var tempRect$2 = new Rectangle();
function generateTextureMatrix(out$1, style, shape, matrix) {
	const textureMatrix = style.matrix ? out$1.copyFrom(style.matrix).invert() : out$1.identity();
...
var GraphicsContextRenderData = class {
	...
	init(maxTextures) {
		this.batcher = new DefaultBatcher({ maxTextures });
...
var _GraphicsContextSystem = class _GraphicsContextSystem$1 {
	...
	*/
	init(options) {
		_GraphicsContextSystem$1.defaultOptions.bezierSmoothness = options?.bezierSmoothness ?? _GraphicsContextSystem$1.defaultOptions.bezierSmoothness;
	}
	/**
...
function buildAdaptiveBezier(points, sX, sY, cp1x, cp1y, cp2x, cp2y, eX, eY, smoothness) {
	const scale = 1;
	const smoothing = Math.min(.99, Math.max(0, smoothness ?? GraphicsContextSystem.defaultOptions.bezierSmoothness));
...
var ShapePath = class {
	...
	* @returns The instance of the current object for chaining.
	*/
	addPath(path$1, transform) {
		this.endPoly();
		if (transform && !transform.isIdentity()) {
			path$1 = path$1.clone(true);
			path$1.transform(transform);
		}
		const shapePrimitives = this.shapePrimitives;
		const start = shapePrimitives.length;
		for (let i = 0; i < path$1.instructions.length; i++) {
			const instruction = path$1.instructions[i];
			this[instruction.action](...instruction.data);
		}
		if (path$1.checkForHoles && shapePrimitives.length - start > 1) {
			let mainShape = null;
	...
	*/
	...
	/** Builds the path. */
	buildPath() {
		const path$1 = this._graphicsPath2D;
	...
	*/
	getLastPoint(out$1) {
		...
		switch (lastInstruction.action) {
			case "moveTo":
			case "lineTo":
				out$1.x = lastInstruction.data[0];
				out$1.y = lastInstruction.data[1];
				break;
			case "quadraticCurveTo":
				out$1.x = lastInstruction.data[2];
				out$1.y = lastInstruction.data[3];
				break;
			case "bezierCurveTo":
				out$1.x = lastInstruction.data[4];
				out$1.y = lastInstruction.data[5];
				break;
			case "arc":
			case "arcToSvg":
				out$1.x = lastInstruction.data[5];
				out$1.y = lastInstruction.data[6];
				break;
			case "addPath":
				lastInstruction.data[0].getLastPoint(out$1);
				break;
		}
		return out$1;
	}
...
var _GraphicsContext = class _GraphicsContext$1 extends eventemitter3_default {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		...
		this._fillStyle = { ..._GraphicsContext$1.defaultFillStyle };
		this._strokeStyle = { ..._GraphicsContext$1.defaultStrokeStyle };
		this._stateStack = [];
		this._tick = 0;
		this._bounds = new Bounds();
	...
	*/
	clone() {
		const clone = new _GraphicsContext$1();
	...
	*/
	...
	set fillStyle(value) {
		this._fillStyle = toFillStyle(value, _GraphicsContext$1.defaultFillStyle);
	}
	/**
	...
	*/
	...
	set strokeStyle(value) {
		this._strokeStyle = toStrokeStyle(value, _GraphicsContext$1.defaultStrokeStyle);
	}
	/**
	...
	*/
	setFillStyle(style) {
		this._fillStyle = toFillStyle(style, _GraphicsContext$1.defaultFillStyle);
		return this;
	}
	/**
	...
	*/
	setStrokeStyle(style) {
		this._strokeStyle = toFillStyle(style, _GraphicsContext$1.defaultStrokeStyle);
		return this;
	}
	...
	*/
	...
	fill(style, alpha) {
		...
		if (this._tick === 0 && lastInstruction && lastInstruction.action === "stroke") path$1 = lastInstruction.data.path;
		else path$1 = this._activePath.clone();
	...
	*/
	stroke(style) {
		let path$1;
		const lastInstruction = this.instructions[this.instructions.length - 1];
		if (this._tick === 0 && lastInstruction && lastInstruction.action === "fill") path$1 = lastInstruction.data.path;
		else path$1 = this._activePath.clone();
		if (!path$1) return this;
		if (style != null) this._strokeStyle = toStrokeStyle(style, _GraphicsContext$1.defaultStrokeStyle);
		this.instructions.push({
			action: "stroke",
			data: {
	...
	*/
	path(path$1) {
		this._tick++;
		this._activePath.addPath(path$1, this._transform.clone());
		return this;
	}
	/**
	* Connects the current point to a new point with a straight line. This method updates the current path.
	...
	*/
	containsPoint(point) {
		...
		for (let k = 0; k < instructions.length; k++) {
			...
			if (!instruction.action || !path$1) continue;
			const style = data.style;
			const shapes = path$1.shapePath.shapePrimitives;
...
var _TextStyle = class _TextStyle$1 extends eventemitter3_default {
	constructor(style = {}) {
		super();
		/**
	...
	*/
	...
	set dropShadow(value) {
		...
		if (value !== null && typeof value === "object") this._dropShadow = this._createProxy({
			..._TextStyle$1.defaultDropShadow,
			...value
		});
		else this._dropShadow = value ? this._createProxy({ ..._TextStyle$1.defaultDropShadow }) : null;
		this.update();
	}
	...
	*/
	...
	reset() {
		const defaultStyle = _TextStyle$1.defaultTextStyle;
...
function getCanvasFillStyle(fillStyle, context, textMetrics, padding = 0) {
	if (fillStyle.texture === Texture.WHITE && !fillStyle.fill) return Color.shared.setValue(fillStyle.color).setAlpha(fillStyle.alpha ?? 1).toHexa();
		...
		const tempMatrix$1 = fillStyle.matrix.copyTo(Matrix.shared);
		tempMatrix$1.scale(fillStyle.texture.frame.width, fillStyle.texture.frame.height);
		pattern.setTransform(tempMatrix$1);
		return pattern;
	} else if (fillStyle.fill instanceof FillPattern) {
		const fillPattern = fillStyle.fill;
		const pattern = context.createPattern(fillPattern.texture.source.resource, "repeat");
		const tempMatrix$1 = fillPattern.transform.copyTo(Matrix.shared);
		tempMatrix$1.scale(fillPattern.texture.frame.width, fillPattern.texture.frame.height);
		pattern.setTransform(tempMatrix$1);
		return pattern;
...
var _DynamicBitmapFont = class _DynamicBitmapFont$1 extends AbstractBitmapFont {
	/**
	...
	constructor(options) {
		...
		*/
		...
		const dynamicOptions = {
			..._DynamicBitmapFont$1.defaultOptions,
			...options
		};
...
function getBitmapTextLayout(chars, style, font, trimEnd) {
	...
	for (let i = 0; i < chars.length + 1; i++) {
		...
		if (/(?:\s)/.test(char) || char === "\r" || char === "\n" || isEnd) {
			...
			if (breakWords && checkIsOverflow(currentLine.width + currentWord.width + nextCharWidth)) {
				nextWord(currentWord);
				nextLine();
			}
...
var PassthroughFilter = class extends Filter {
	constructor() {
		const gpuProgram$2 = GpuProgram.from({
			vertex: {
				source,
		...
		const glProgram$2 = GlProgram.from({
			vertex: vertex$1,
			fragment: fragment$1,
			name: "passthrough-filter"
		});
...
var Graphics = class Graphics extends ViewContainer {
	...
	*/
	constructor(options) {
		...
		if (!context) this._context = this._ownedContext = new GraphicsContext();
...
var _MeshGeometry = class _MeshGeometry$1 extends Geometry {
	constructor(...args) {
		let options = args[0] ?? {};
		...
		options = {
			..._MeshGeometry$1.defaultOptions,
			...options
		};
...
var CanvasTextGeneratorClass = class {
	...
	*/
	getCanvasAndContext(options) {
		...
		const frame = style.trim ? getCanvasBoundingBox({
	...
	_renderTextToCanvas(text, style, padding, resolution, canvasAndContext) {
		...
		for (let i = 0; i < passesCount; ++i) {
			...
			if (isShadowPass) {
				...
				if (style._stroke?.width) {
					const strokePadding = style._stroke.width * .5 + padding * 2;
			...
			for (let i2 = 0; i2 < lines.length; i2++) {
				...
				if (style._stroke?.width) this._drawLetterSpacing(lines[i2], style, canvasAndContext, linePositionX + padding, linePositionY + padding - dsOffsetText, true);
				if (style._fill !== void 0) this._drawLetterSpacing(lines[i2], style, canvasAndContext, linePositionX + padding, linePositionY + padding - dsOffsetText);
			}
		}
	}
	/**
...
var _PlaneGeometry = class _PlaneGeometry$1 extends MeshGeometry {
	constructor(...args) {
		super({});
		let options = args[0] ?? {};
	...
	*/
	build(options) {
		options = {
			..._PlaneGeometry$1.defaultOptions,
			...options
		};
...
//#endregion
//#region node_modules/pixi.js/lib/scene/particle-container/shared/GlParticleContainerPipe.mjs
...
//#endregion
//#region node_modules/pixi.js/lib/scene/particle-container/shared/GpuParticleContainerPipe.mjs
...
var _NineSliceGeometry = class _NineSliceGeometry$1 extends PlaneGeometry {
	constructor(options = {}) {
		options = {
			..._NineSliceGeometry$1.defaultOptions,
			...options
		};
		super({
			width: options.width,
...
//#endregion
//#region node_modules/pixi.js/lib/scene/text-bitmap/BitmapTextPipe.mjs
...
var BitmapTextPipe = class {
	...
	_updateContext(bitmapText, proxyGraphics) {
		...
		if (bitmapFont.distanceField.type !== "none") {
			if (!context.customShader) context.customShader = new SdfShader(this._renderer.limits.maxBatchableTextures);
		}
	...
	const fontPromises = fontFamilies.filter((fontFamily) => Cache.has(`${fontFamily}-and-url`)).map((fontFamily) => {
		if (!FontStylePromiseCache.has(fontFamily)) {
			...
			entries.forEach((entry) => {
				const url = entry.url;
				const out$1 = entry.faces.map((face) => ({
					weight: face.weight,
					style: face.style
				}));
				promises.push(...out$1.map((style) => loadFontCSS({
...
var HTMLTextSystem = class {
	...
	*/
	getManagedTexture(text) {
		...
		const promise$1 = this._buildTexturePromise(text).then((texture) => {
...
var BatchableText = class extends BatchableSprite {
	...
	destroy() {
		...
		if (canvasText.getReferenceCount(this.currentKey) > 0) canvasText.decreaseReferenceCount(this.currentKey);
		else if (this.texture) canvasText.returnTexture(this.texture);
		this._renderer.runners.resolutionChange.remove(this);
		this._renderer = null;
	}
};
...
var CanvasTextPipe = class {
	...
	addRenderable(text, instructionSet) {
		...
		if (text._didTextUpdate) {
			...
			if (batchableText.currentKey !== text.styleKey || text.resolution !== resolution) this._updateGpuText(text);
...
var CanvasTextSystem = class {
	...
	*/
	returnTexture(texture) {
		const source$1 = texture.source;
		source$1.resource = null;
		source$1.uploadMethodId = "unknown";
		source$1.alphaMode = "no-premultiply-alpha";
		TexturePool.returnTexture(texture, true);
	}
	/**
	* Renders text to its canvas, and updates its texture.
...
//#endregion
export { AbstractBitmapFont, AlphaMask, BatchableGraphics, BatchableHTMLText, BatchableMesh, BatchableText, BitmapFontManager, BitmapTextGraphics, BitmapTextPipe, Cache, CanvasTextGenerator, CanvasTextMetrics, CanvasTextPipe, CanvasTextSystem, Circle, ColorMask, DynamicBitmapFont, Ellipse, FillGradient, FillPattern, FilterPipe, FilterSystem, FontStylePromiseCache, GlParticleContainerAdaptor, GlParticleContainerPipe, GpuGraphicsContext, GpuParticleContainerAdaptor, GpuParticleContainerPipe, Graphics, GraphicsContext, GraphicsContextRenderData, GraphicsContextSystem, GraphicsGpuData, GraphicsPath, GraphicsPipe, HTMLTextPipe, HTMLTextRenderData, HTMLTextSystem, ImageSource, LoaderParserPriority, MeshGeometry, MeshGpuData, MeshPipe, NineSliceGeometry, NineSliceSpriteGpuData, NineSliceSpritePipe, ParticleBuffer, ParticleContainerPipe, ParticleShader, PassthroughFilter, PlaneGeometry, Polygon, QuadGeometry, ResizePlugin, Resolver, RoundedRectangle, SVGParser, SdfShader, ShapePath, Spritesheet, StencilMask, TextStyle, TickerPlugin, TilingSpriteGpuData, TilingSpritePipe, TilingSpriteShader, VideoSource, addMaskBounds, addMaskLocalBounds, appendSVGPath, applyMatrix, autoDetectSource, buildAdaptiveBezier, buildAdaptiveQuadratic, buildArc, buildArcTo, buildArcToSvg, buildCircle, buildContextBatches, buildEllipse, buildLine, buildPixelLine, buildPolygon, buildRectangle, buildRoundedRectangle, buildSimpleUvs, buildTriangle, buildUvs, calculatePathArea, checkForNestedPattern, closePointEps, convertToList, copySearchParams, createIndicesForQuads, createStringVariations, curveEps, detectVideoAlphaMode, extractFontFamilies, extractSubpaths, extractSvgUrlId, fontStringFromTextStyle, fragment, fragment$1, generateParticleUpdateFunction, generateTextureMatrix, getBitmapTextLayout, getCanvasBoundingBox, getCanvasFillStyle, getFillInstructionData, getFontCss, getGlobalRenderableBounds, getOrientationOfPoints, getPo2TextureFromSource, getSVGUrl, getTemporaryCanvasFromImage, getUrlExtension, isSafari, isSingleItem, loadFontAsBase64, loadFontCSS, loadSVGImage, localUniformMSDFBit, localUniformMSDFBitGl, mSDFBit, mSDFBitGl, measureHtmlText, parseSVGDefinitions, parseSVGFloatAttribute, parseSVGPath, parseSVGStyle, path, resolveCharacters, resourceToTexture, roundedShapeArc, roundedShapeQuadraticCurve, setPositions, setUvs, shapeBuilders, source, spritesheetAsset, squaredDistanceToLineSegment, styleAttributes, textureFrom, tilingBit, tilingBitGl, toFillStyle, toStrokeStyle, transformVertices, triangulateWithHoles, updateTextBounds, vertex, vertex$1, wgsl };
//# sourceMappingURL=CanvasTextSystem-DxIqgCsb.js.map